...
import (
	...
	"github.com/cicutaviroza/hbooking/internal/hbooking/domain"
)
...
func (h *Handlers) CreateBooking(c *gin.Context) {
	...
	endAt, err := time.Parse("02-02-2006 15:04", req.EndAt)
	...
	if ValidationFailed(c, booking.EndAt.Before(now), "end_at is before begin_at") {
		return
	}
	...
	if ValidationFailed(c,
		...
		"Invalid booking duratipon: must be in range [30, 4h]",
	) {
		return
	}
